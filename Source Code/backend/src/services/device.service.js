...
class DeviceService {
    ...
    async getDeviceById(id) {
        ...
        if (!device)
            throw { statusCode: 404, message: 'Thiết bị không tồn tại' };
    ...
    async createDevice(name, board, sourceCode) {
        ...
        if (!Device.isValidName(name))
            throw { statusCode: 400, message: 'Tên thiết bị không hợp lệ (chỉ chứa chữ, số, gạch ngang và gạch dưới, tối đa 100 ký tự)' };
        ...
        try {
            ...
            throw { statusCode: 500, message: 'Biên dịch firmware thất bại: ' + error.message };
        }
    ...
    async updateVersion(deviceId, sourceCode, userId) {
        ...
        let result;
        try {
            result = await buildFirmware(sourceCode, device.name, device.board, currentVersion);
        } catch (error) {
            throw { statusCode: 500, message: 'Biên dịch firmware thất bại: ' + error.message };
        }

        // Cập nhật database
        await deviceRepo.updateVersion(deviceId, result.version, device.total_versions + 1);
