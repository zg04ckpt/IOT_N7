...
class InvoiceService {
    ...
    async getInvoicesByDateRange(startDate, endDate, page, size) {
        if (!startDate || !endDate)
            throw { statusCode: 400, message: 'Vui lòng cung cấp khoảng thời gian (startDate, endDate)' };
    ...
    async createSessionInvoice(sessionId, userId) {
        ...
        if (!session)
            throw { statusCode: 404, message: 'Session không tồn tại' };

        if (!session.check_in || !session.check_out)
            throw { statusCode: 400, message: 'Session chưa hoàn thành (thiếu check-in hoặc check-out)' };

        const amount = Invoice.calculateSessionFee(session.check_in, session.check_out);
